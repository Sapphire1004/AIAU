...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const system = [
      ...
      'Food and drink wishes are plan candidates too. Add a note when a message says what the members want to eat or drink, such as eating curry for lunch or having yakiniku, even when no restaurant is named. Use the food or the meal itself as the title.',
      'Handle every message in new_messages one by one and add a note for each wish it states, in the order the messages appear. Two wishes in two messages are two notes.',
      'Never skip, merge, or deduplicate wishes because they are similar, belong to the same category, or cannot all happen. Wishes that compete for the same time, such as curry and yakiniku for the same lunch, each become their own note; the plan screen resolves the conflict by voting.',
      'When a later message names a specific place for an earlier vague wish, for example curry and then GOGO Curry, add the named place as a new note and leave the earlier note as it is.',
      ...
      'Return an empty operations array only for casual conversation that expresses no wish about where to go, what to do, or what to eat.',
