...
AIAU は、旅行やお出かけの相談をチャットで進めながら、AI がアイデアを付箋として整理し、投票可能なタイムラインとカレンダーへつなげる共同旅行プランナーです。
...
flowchart TB
  ...
  Functions -. optional .-> LLM[JSON対応LLM API]
```
...
LLM設定がないローカル環境では、`extract-notes` と `generate-plan` は決定的なフォールバック処理を利用できます。
...
`.env.local` をエディタで開き、`npx supabase status -o env` の `API_URL` を `VITE_SUPABASE_URL`、`PUBLISHABLE_KEY` を `VITE_SUPABASE_PUBLISHABLE_KEY` に設定します。
...
LLM APIを使う場合は、Git管理対象外の `supabase/.env.local` を作成します。
...
その場合はenvファイルを明示して起動します。

```bash
npx supabase functions serve --env-file supabase/.env.local
```

未設定でもフォールバック処理で主要フローを確認できます。
...
| 変数 | 必須 | 用途 |
| --- | --- | --- |
| `LLM_API_URL` | LLM使用時 | OpenAI互換のJSON生成API URL |
| `LLM_API_KEY` | LLM使用時 | LLM API認証キー |
| `LLM_MODEL` | LLM使用時 | 呼び出すモデル名 |
| `VAPID_PUBLIC_KEY` | Web Push使用時 | VAPID公開鍵 |
| `VAPID_PRIVATE_KEY` | Web Push使用時 | VAPID秘密鍵 |
| `VAPID_SUBJECT` | Web Push使用時 | `mailto:` またはHTTPSの連絡先 |

Hosted Supabaseでは、DashboardでAnonymous Sign-Insを有効化し、Edge Function Secretsを設定してください。Clientへ渡すのはProject URLとPublishable Keyだけです。Service Role Key、LLM API Key、VAPID Private Keyをフロントエンドへ配置してはいけません。
...
- **Secret分離**: Service Role Key、LLM Key、VAPID Private KeyはEdge Functionsだけで利用
